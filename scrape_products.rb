...
def more_pages2go flag, current_page_number
	if flag == nil
		return false
	end
...
# Faz o método transliterate (que tira os acentos
# de uma palavra) funcionar.
...
for i in 0..(total_categories_number - 1)
	...
	system("mkdir #{format_text(category)}")
	...
	loop do
		...
		for j in 0..(number_of_links - 1)
			...
			if theres_a_product(category_page.links[j].text)
				...
				#system("mkdir #{format_text(product.category)}/#{format_text(product.name)}")
		...
		if !more_pages2go(aux, current_page_number)
			break
		end
...
# EXEMPLO PARA FORMATAÇÃO DA DESCRIÇÃO
# https://www.cupidodistribuidora.com.br/produto,8512,pack-com-10-unidades-do-produto-pomada-anestesica-sensory-7-5g-sexy-fantasy.html